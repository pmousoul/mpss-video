...
static addrinfo *getAddressInfo(const char *host, const char *service, SocketAddress::AddressType atype)
...
SocketAddress::SocketAddress(const char *host, const char *service, AddressType atype)
	throw(SocketException) {
...
SocketAddress::SocketAddress(const char *host, in_port_t port, AddressType atype)
	throw(SocketException) {
...
vector<SocketAddress> SocketAddress::lookupAddresses(const char *host, const char *service, AddressType atype)
	throw(SocketException) {
	...
	for (addrinfo *curAddr = servAddrs; curAddr != NULL; curAddr = curAddr->ai_next)
		addrList.push_back(SocketAddress((sockaddr *)(curAddr->ai_addr), curAddr->ai_addrlen));
...
vector<SocketAddress> SocketAddress::lookupAddresses(const char *host, in_port_t port, AddressType atype)
	throw(SocketException) {
...
// Currently there is check if the socket is closed properly
// See TCP/IP Sockets in C book page 23 for more details
void Socket::close() {
...
void Socket::createSocket(const SocketAddress &address, int type, int protocol) throw(SocketException) {
...
size_t CommunicatingSocket::send(const void *buffer, int bufferLen) throw(SocketException) {

	int rtn = ::send(sockDesc, (raw_type *)buffer, bufferLen, 0);
	if (rtn < 0) {
...
size_t CommunicatingSocket::recvFully(void *buffer, int bufferLen) throw(SocketException) {
	...
	while (len > 0 && rcount + len < bufferLen) {
		...
		rcount += len;
		len = ::recv(sockDesc, (raw_type *) (((char *) buffer) + rcount), bufferLen - rcount, 0);

	}

	if (len < 0)
		throw SocketException("Receive failed (recvFully())");
...
// Subclass of basic_streambuf for reading and writing to instances of TCPSocket.
...
public:
	...
	SocketStreamBuffer(TCPSocket *sock) {
		...
		this->setg(inBuffer, inBuffer + sizeof(inBuffer), inBuffer + sizeof(inBuffer));
...
TCPSocket::TCPSocket(const char *foreignAddress, in_port_t foreignPort) throw(SocketException) {
	...
	// Get linked list of remote addresses
	addrinfo *servAddrs;
	int rtnVal = getaddrinfo(foreignAddress, service, &addrCriteria, &servAddrs);
	if (rtnVal != 0)
		throw SocketException("getaddrinfo() failed", gai_strerror(rtnVal));

	// Try each address until we get one that we can connect.
	for (addrinfo *curAddr = servAddrs; curAddr != NULL && sockDesc < 0; curAddr = curAddr->ai_next) {

		// Try to make a new socket.
		if ((sockDesc = socket(curAddr->ai_family, curAddr->ai_socktype, curAddr->ai_protocol)) >= 0 ) {
...
void TCPSocket::bind(const SocketAddress &localAddress) throw(SocketException) {
	...
	if (::bind(sockDesc, localAddress.getSockaddr(), localAddress.getSockaddrLen()) < 0)
		throw SocketException(string("Call to bind() failed :  ") + strerror(errno));
...
void TCPSocket::connect(const SocketAddress &foreignAddress) throw(SocketException) {
	...
	if (::connect(sockDesc, foreignAddress.getSockaddr(), foreignAddress.getSockaddrLen()) < 0)
...
int findTCPServerSocketDesc(const char *localAddr, const char *localPort) {
	...
	int desc;
	if ((desc = socket(servAddr->ai_family, servAddr->ai_socktype, servAddr->ai_protocol)) < 0)
...
TCPServerSocket::TCPServerSocket(in_port_t localPort, int queueLen) throw(SocketException) {
...
void UDPSocket::bind(const SocketAddress &localAddress) throw(SocketException) {
...
void UDPSocket::connect(const SocketAddress &foreignAddress) throw(SocketException) {
...
void UDPSocket::sendTo(const void *buffer, int bufferLen, const SocketAddress &foreignAddress)
	throw(SocketException) {
...
int UDPSocket::recvFrom(void *buffer, int bufferLen, SocketAddress &sourceAddress)
	throw(SocketException) {
...
void UDPSocket::create(const SocketAddress &foreignAddress) throw(SocketException) {

	if (sockDesc < 0)
		createSocket(foreignAddress, SOCK_DGRAM, IPPROTO_UDP);

	if (sockDesc < 0)
		throw SocketException("UDP socket creation failed");

}

void UDPSocket::nonBlock() throw(SocketException) {

	int flags;
	if ((flags = fcntl(sockDesc, F_GETFL, 0)) < 0)
		throw SocketException("Could not change socket to non-blocking");

	if (fcntl(sockDesc, F_SETFL, flags | O_NONBLOCK) < 0)
		throw SocketException("Could not change socket to non-blocking");

}

int UDPSocket::recvFromNonBlock(void *buffer, int bufferLen, SocketAddress &sourceAddress) {
...
int UDPSocket::sendToNonBlock(const void *buffer, int bufferLen, const SocketAddress &foreignAddress) {

	// Write out the whole buffer as a single message.
	int rtn = sendto(sockDesc, (raw_type *) buffer, bufferLen, 0, foreignAddress.getSockaddr(), foreignAddress.getSockaddrLen());
