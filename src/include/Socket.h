...
#include <fcntl.h> // Used for setting socket to nonblocking
...
public:
	 ...
	 * default constructor, the SocketAddress is created in an
...
public:
	 ...
	 * address and port. This interface is provided as a convience
...
private:
	...
	// iostream associated with this socket, or NULL if it doesn't have one.
...
public:
	...
	void create(const SocketAddress &foreignAddress) throw(SocketException);

	// Convert the socket to non-blocking
	void nonBlock() throw(SocketException);

	/*
	 * The non-blocking version of recvFrom()
	 * It is allowed to fail without throwing an exception
	 * Application programmer must handle the failure
	 */
	int recvFromNonBlock(void *buffer, int bufferLen, SocketAddress &sourceAddress);

	/*
	 * The non-blocking version of sendTo()
	 * It is allowed to fail without throwing an exception
	 * Application programmer must handle the failure
	 */
	int sendToNonBlock(const void *buffer, int bufferLen, const SocketAddress &foreignAddress);
